...
class _SplashScreenState extends ConsumerState<SplashScreen>
  ...
  static const Duration _failsafeTimeout = Duration(seconds: 12);
  ...
  void initState() {
    ...
    _bootstrapFuture = ref.read(authActionProvider).bootstrapSession();
    ...
    _failsafeTimer = Timer(_failsafeTimeout, () {
      unawaited(_forceFallbackNavigation());
    });
  }
  ...
  Future<void> _forceFallbackNavigation() async {
    if (_didNavigate || !mounted) {
      return;
    }

    developer.log(
      'Splash failsafe timeout reached; routing to login.',
      name: 'SplashScreen',
    );
    await _navigateSafely(RouteNames.login);
  }

  Future<void> _navigateSafely(String routeName) async {
    if (_didNavigate || !mounted) {
      return;
    }
    ...
    try {
      ...
      developer.log(
        'Splash navigation failed: $error',
        name: 'SplashScreen',
        error: error,
  ...
  Future<void> _navigateNext() async {
    ...
    try {
      didBootstrap = await _bootstrapFuture.timeout(
        _bootstrapTimeout,
        onTimeout: () {
          developer.log(
            'Auth bootstrap timed out on splash.',
            name: 'SplashScreen',
          );
          return false;
        },
      ...
      developer.log(
        'Auth bootstrap failed on splash: $error',
        name: 'SplashScreen',
        error: error,
        stackTrace: stackTrace,
      );
    ...
    final isAuthenticated = didBootstrap && ref.read(isAuthenticatedProvider);
    final target = isAuthenticated ? RouteNames.dashboard : RouteNames.login;
    await _navigateSafely(target);
  }
