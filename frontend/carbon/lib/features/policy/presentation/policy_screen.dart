...
class _PolicyScreenState extends ConsumerState<PolicyScreen> {
  Future<void> _onSharePolicy() async {
    const policyUrl = 'http://107.21.121.84:8001/policy/details';
    await Clipboard.setData(const ClipboardData(text: policyUrl));
    if (!mounted) {
      return;
    }
    AppSnackbar.show(context, 'Policy link copied for sharing.');
  }

  void _onDownloadPolicy() {
    AppSnackbar.show(
      context,
      'Policy download will be enabled with document endpoint integration.',
    );
  }
  ...
  Future<void> _onDeclinePolicy() async {
    ...
    AppSnackbar.show(context, 'Policy declined. Returning to dashboard.');
  ...
  Widget build(BuildContext context) {
    ...
    final appBarActions = <Widget>[
      IconButton(
        ...
        onPressed: _onDownloadPolicy,
        icon: const Icon(Icons.download_outlined),
      ),
      IconButton(
        tooltip: 'Share Policy',
        onPressed: _onSharePolicy,
          ...
          children: <Widget>[
            ...
            if (policyAsync.hasError)
              Container(
                ...
                child: Text(
                  'Live policy data could not be loaded. Showing fallback policy details.',
                  style: textTheme.bodyMedium?.copyWith(
            ...
            FilledButton.icon(
              onPressed: isSubmitting ? null : _onAcceptPolicy,
              icon: isSubmitting
                  ? SizedBox(
                      width: 18,
                      height: 18,
